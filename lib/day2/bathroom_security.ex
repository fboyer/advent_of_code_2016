defmodule AdventOfCode2016.BathroomSecurity do
  def get_bathroom_code(instructions, opts \\ [keypad_type: :normal]) do
    instructions
    |> String.split("\n", trim: true)
    |> Enum.reduce([], fn(instruction_line, keys) ->
      keys
      |> List.insert_at(-1, get_next_key(opts[:keypad_type], instruction_line, keys))
    end)
    |> Enum.join
  end

  def get_next_key(keypad_type, instruction_line, previous_keys) do
    last_key =
